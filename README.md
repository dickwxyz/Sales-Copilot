...
本系统将经过多年市场验证的教培销售方法论（客户决策心理四阶段模型、SPIN 销售法则、竞品差异化策略）编译为知识库，通过 DeepSeek API 一次性完成客户心理阶段判定、画像提取、信息完整性评估、策略建议生成和三套话术（专业型/亲和型/高效型）的生成。系统搭载关键字注入防护模块，最大限度降低提示泄露风险。
